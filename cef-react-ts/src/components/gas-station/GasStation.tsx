...
import {Button, Card, Col, Divider, InputNumber, Result, Row, Slider, Space, Statistic, Typography} from "antd";
            ...
            <Card title={"Заправка"} extra={<Button icon={<CloseOutlined/>} onClick={()=>Client.closeWindow()}/>}>
                <div style={{width: '50vw', height: '50vh'}}>
                    {maxFuel <= 0 ?
                        <Result
                            status="warning"
                            title="У вас полный бак!"
                            extra={
                                <Button type="primary" key="console" onClick={()=>Client.closeWindow()}>
                                    Закрыть
                                </Button>
                            }
                        />
